...
class CarsService {
  async deleteCar(carData, userInfo) {
    const car = await this.getCar(carData.carId)
    if (userInfo.id != car.sellerId.toString()) {
      throw new UnAuthorized('That aint your car')
    }
    await dbContext.Cars.findByIdAndDelete(carData.carId)
