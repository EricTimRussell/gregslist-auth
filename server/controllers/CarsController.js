...
export class CarsController extends BaseController {
  ...
  async editCar(req, res, next) {
    try {
      const car = await carsService.editCar(req.params, req.userInfo)
      res.send(car)
    } catch (error) {
      next(error)
    }
  ...
  async deleteCar(req, res, next) {
    try {
      const car = await carsService.deleteCar(req.params, req.userInfo)
      res.send("This car has been delorted")
    } catch (error) {
      next(error)
    }
  }
